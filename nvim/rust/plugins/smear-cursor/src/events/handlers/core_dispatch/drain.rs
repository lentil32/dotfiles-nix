...
use super::super::super::runtime::MAX_CHAINED_SCHEDULED_WORK_ITEMS_PER_EDGE;
use super::super::super::runtime::MAX_SCHEDULED_WORK_ITEMS_PER_EDGE;
use super::super::super::runtime::MIN_SCHEDULED_WORK_ITEMS_PER_EDGE;
use super::super::super::runtime::NeovimEffectExecutor;
use super::super::super::runtime::ScheduledEffectDrainEntry;
use super::super::super::runtime::ScheduledEffectQueueState;
use super::super::super::runtime::ScheduledWorkItem;
use super::super::super::runtime::ScheduledWorkUnit;
...
use super::super::super::runtime::with_dispatch_queue;
...
use super::execute::execute_shell_only_step;
...
pub(super) fn scheduled_drain_budget() -> usize {
    ...
    with_dispatch_queue(|queue| {
        ...
        if thermal == RenderThermalState::Hot && queue_has_only_shell_only_agendas(queue) {
...
fn queue_has_only_shell_only_agendas(queue: &ScheduledEffectQueueState) -> bool {
    !queue.items.is_empty()
        && queue
            .items
            .iter()
            .all(|item| matches!(item, ScheduledWorkItem::ShellOnlyAgenda(_)))
}
...
pub(super) fn scheduled_drain_budget_for_hot_effect_only_snapshot(
    ...
    // CONTEXT: once Hot queue state has collapsed down to shell-only metric/redraw
...
fn should_continue_shell_only_follow_up_chain(
    ...
    with_dispatch_queue(|queue| {
        ...
        // metric/redraw tail work queued. Once that remaining queue has collapsed to shell-only
        // agendas, continue immediately in the same callback instead of paying an extra scheduled
        // shell edge for the tail.
        !queue.items.is_empty()
            && queue
                .items
                .iter()
...
pub(super) fn drain_scheduled_work_with_executor(
    ...
    loop {
        ...
        while remaining_budget > 0 {
            let Some(item) = with_dispatch_queue(ScheduledEffectQueueState::pop_work_unit) else {
                with_dispatch_queue(|queue| {
                    queue.drain_scheduled = false;
                });
                if drained_items > 0 {
        ...
        // Once Hot drains the full pre-existing snapshot, any remaining shell-only batches were
        // spawned mid-pass by reducer follow-ups inside this callback. Continue through a bounded
        // number of those low-risk waves here. Observation, timer, cleanup, and queued probe work
        // still remain deferred; only the single cursor-color probe attached directly to an
        // ObservationBaseCollected follow-up can execute inline before that queue hop exists.
        if should_continue_shell_only_follow_up_chain(
            snapshot,
    ...
    let has_more_items = with_dispatch_queue(|queue| {
