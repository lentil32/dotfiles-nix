...
impl ScrollShiftSummary {
    fn from_scroll_shift(scroll_shift: ScrollShift) -> Self {
        Self {
            shift: RoundedScalar::from_f64(scroll_shift.shift),
...
fn render_target_cell_presentation(presentation: TargetCellPresentation) -> &'static str {
    match presentation {
        ...
        TargetCellPresentation::OverlayBlockCell => "overlay",
    }
}
...
mod trajectory_goldens {
    ...
    #[test]
    fn surface_retarget_with_overlay_when_cross_window_jump_bridges_are_disabled() {
        let mut state = RuntimeState::default();
        state.config.delay_event_to_smear = 0.0;
        state.config.simulation_hz = 240.0;
        state.config.max_simulation_steps_per_frame = 16;
        state.config.hide_target_hack = true;
        state.config.smear_between_windows = true;
        state.config.cross_window_jump_bridges = false;
        state.refresh_render_static_config();
        let seed_state = state.clone();

        let steps = [
            external_step(5.0, 6.0, 100.0, 51, 10, 20),
            external_step(20.0, 30.0, 108.0, 52, 11, 20),
            animation_tick_step(20.0, 30.0, 116.0, 53, 11, 20),
        ];
