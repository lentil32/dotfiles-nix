...
# SMEAR_BUFFER_PERF_MODE=fast \
# SMEAR_PLANNER_COMPILE_MODE=reference \
...
#
# Scenario sets:
# - `single`: run only `SMEAR_SCENARIO_NAME`.
# - `matrix`: run the existing burst-delay stress matrix.
# - `presets`: run named workload presets such as large-buffer, planner-heavy, extmark-heavy,
#   and conceal-heavy.
# - `all`: run both `matrix` and `presets`.
...
run_burst_delay_matrix() {
  local heavy_burst_stress_iterations
  local heavy_burst_stress_rounds
  local short_settle_wait_ms
  local long_settle_wait_ms
  local cold_wait_timeout_ms
  local delay_event_to_smear_on
  local delay_event_to_smear_off
...
run_preset_matrix() {
  ...
  for scenario_name in "${preset_scenarios[@]}"; do
    run_perf_scenario \
      "${scenario_name}" \
...
case "${scenario_set}" in
  ...
  matrix)
    run_burst_delay_matrix
    ;;
  presets|adaptive_policy|probe_pipeline)
    run_preset_matrix
    ;;
  all)
    run_burst_delay_matrix
