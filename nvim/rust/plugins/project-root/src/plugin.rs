...
use crate::buffer::NormalizedPathKey;
use crate::buffer::cached_root_by_key;
use crate::buffer::debug_log;
use crate::buffer::get_path_from_buffer;
use crate::buffer::normalized_path_key;
use crate::buffer::set_buf_root;
...
impl ProjectRootContext {
    ...
    fn state_lock(&self) -> nvimrs_nvim_oxi_utils::state::StateGuard<'_, State> {
...
fn get_project_root(context: &ProjectRootContext) -> Result<Option<String>> {
    ...
    let normalized = nvimrs_nvim_utils::path::normalize_path(&cwd);
    debug_log(|| {
