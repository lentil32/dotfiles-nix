...
#[test]
fn cleanup_policy_keep_warm_rearms_until_penalty_window_elapses() {
...
#[test]
fn cleanup_policy_soft_clear_rearms_until_hard_purge_horizon() {
    let input = CleanupPolicyInput {
        idle_ms: 800,
        soft_cleanup_delay_ms: 200,
        hard_cleanup_delay_ms: 3_000,
...
#[test]
fn external_retarget_enters_settling_until_deadline() {
    let mut state = RuntimeState::default();
...
#[test]
fn settling_to_tracking_when_target_returns_to_current_cursor() {
    let mut state = RuntimeState::default();
    state.config.delay_event_to_smear = 40.0;
...
#[test]
fn stop_hysteresis_requires_consecutive_enter_frames_before_tail_drain_starts() {
    let mut state = RuntimeState::default();
    ...
    let mut drained_at_tick: Option<u32> = None;
    // Surprising: stateful springs make settle timing highly trajectory-dependent, so this
    // test guards lifecycle invariants instead of exact frame counts.
    for tick in 3_u32..=128_u32 {
    ...
    let drained_at_tick =
        drained_at_tick.expect("animation should eventually settle and start draining");
    assert!(
    ...
    assert!(
        state.is_draining(),
        "settled animation should enter drain, remaining_steps={}",
        state.drain_steps_remaining()
    );
}

#[test]
fn tail_drain_emits_idle_planner_steps_then_clears_explicitly() {
    let mut state = RuntimeState::default();
    ...
    let mut started_drain = false;
    let mut saw_idle_planner_steps = false;
    let mut cleared_after_drain = false;
    ...
    assert!(started_drain, "expected animation to enter tail drain");
    assert!(
    ...
    assert!(
        cleared_after_drain,
        "expected drain to finish with an explicit clear-all"
    );
}
...
fn settled_external_noop_after_tail_drain_keeps_cleanup_scheduled() {
    ...
    let settled_at_ms = settled_at_ms.expect("expected tail drain to complete");
...
#[test]
fn rapid_external_retargets_while_animating_keep_scheduling() {
    let mut state = RuntimeState::default();
    state.config.delay_event_to_smear = 0.0;
    ...
    assert!(matches!(render_action(&retarget_a), RenderAction::Draw(_)));
    assert!(retarget_a.should_schedule_next_animation);
    ...
    assert!(matches!(render_action(&retarget_b), RenderAction::Draw(_)));
    assert!(retarget_b.should_schedule_next_animation);
...
#[test]
fn buffer_smear_flag_combination_controls_buffer_switch_clears() {
    ...
    for (smear_between_windows, smear_between_buffers) in combos {
        ...
        state.config.smear_between_buffers = smear_between_buffers;
        ...
        let buffer_switched = reduce_cursor_event(
            ...
            CursorEventContext {
                ...
                now_ms: 120.0,
                seed: 99,
                cursor_location: CursorLocation::new(10, 999, 1, 1),
                scroll_shift: None,
            },
            EventSource::External,
        );
        let expected_buffer_clear = !smear_between_buffers;
        assert_eq!(
            matches!(render_action(&buffer_switched), RenderAction::ClearAll),
            expected_buffer_clear,
            "buffer-switch policy mismatch for smear_between_windows={smear_between_windows} smear_between_buffers={smear_between_buffers}",
        );
        if expected_buffer_clear {
            assert_eq!(
...
#[test]
fn window_change_can_keep_head_motion_smooth_while_starting_new_trail_stroke() {
    let mut state = RuntimeState::default();
    state.config.delay_event_to_smear = 0.0;
    ...
    let frame =
        draw_frame(&transition).expect("window hop should draw when inter-window smear is enabled");
    assert!(matches!(render_action(&transition), RenderAction::Draw(_)));
    assert!(transition.should_schedule_next_animation);
...
#[test]
fn buffer_change_can_keep_head_motion_smooth_while_starting_new_trail_stroke() {
    let mut state = RuntimeState::default();
    state.config.delay_event_to_smear = 0.0;
    state.config.smear_between_windows = true;
    state.config.smear_between_buffers = true;
    ...
    let frame =
        draw_frame(&transition).expect("buffer hop should draw when inter-buffer smear is enabled");
    assert!(matches!(render_action(&transition), RenderAction::Draw(_)));
    assert!(transition.should_schedule_next_animation);
    assert!(frame.trail_stroke_id > before_stroke_id);
    assert_eq!(state.trail_stroke_id(), frame.trail_stroke_id);
...
#[test]
fn cross_window_large_move_classifies_as_discontinuous_jump_and_records_launch_cue() {
    let mut state = RuntimeState::default();
    state.config.delay_event_to_smear = 0.0;
...
#[test]
fn same_window_and_cross_window_large_jumps_share_cue_creation_semantics() {
    let mut same_window = RuntimeState::default();
    same_window.config.delay_event_to_smear = 0.0;
    same_window.config.jump_cue_min_display_distance = 8.0;
    let _ = reduce_cursor_event(
        &mut same_window,
        "n",
        event(5.0, 6.0),
    ...
    let same_window_transition = reduce_cursor_event(
        &mut same_window,
        "n",
    ...
    let mut cross_window = RuntimeState::default();
    cross_window.config.delay_event_to_smear = 0.0;
    cross_window.config.jump_cue_min_display_distance = 8.0;
    let _ = reduce_cursor_event(
        &mut cross_window,
        "n",
        event(5.0, 6.0),
    ...
    let cross_window_transition = reduce_cursor_event(
        &mut cross_window,
        "n",
        CursorEventContext {
            row: 5.0,
    ...
    let same_window_cues = same_window.active_jump_cues();
    let cross_window_cues = cross_window.active_jump_cues();
    assert_eq!(
    ...
    assert_eq!(same_window_cues.len(), 1);
    assert_eq!(cross_window_cues.len(), 1);
    assert_eq!(same_window_cues[0].phase, JumpCuePhase::Launch);
    assert_eq!(cross_window_cues[0].phase, JumpCuePhase::Launch);
    ...
    assert_eq!(
        same_window_cues[0].duration_ms,
        cross_window_cues[0].duration_ms
    );
    assert_eq!(same_window_cues[0].strength, cross_window_cues[0].strength);
}
...
fn repeated_cross_window_jumps_cap_cue_chain_length() {
    ...
    let _ = reduce_cursor_event(
        ...
        CursorEventContext {
            ...
            now_ms: 160.0,
            seed: 101,
            cursor_location: CursorLocation::new(777, 20, 1, 1),
            scroll_shift: None,
        },
        EventSource::External,
    );
...
#[test]
fn jump_class_retarget_advances_trail_stroke_id_and_clears_current_plan() {
    let mut state = RuntimeState::default();
    state.config.delay_event_to_smear = 0.0;
...
#[test]
fn cmdline_external_events_progress_after_settle_tick() {
    let mut state = RuntimeState::default();
    state.config.smear_to_cmd = true;
...
#[test]
fn insert_mode_can_force_immediate_snap_without_disabling_insert_smear() {
    let mut state = RuntimeState::default();
...
#[test]
fn cmdline_boundary_snap_is_immediate_but_intra_cmdline_motion_still_animates() {
    let mut state = RuntimeState::default();
    state.config.smear_to_cmd = true;
...
#[test]
fn animation_tick_retarget_updates_target_and_keeps_animation_frames() {
    let mut state = RuntimeState::default();
    let _ = reduce_cursor_event(&mut state, "n", event(5.0, 6.0), EventSource::External);
    let _ = reduce_cursor_event(
        &mut state,
...
#[test]
fn trajectory_golden_rapid_horizontal_motion() {
    let mut state = RuntimeState::default();
    state.config.delay_event_to_smear = 0.0;
    state.config.simulation_hz = 240.0;
    state.config.max_simulation_steps_per_frame = 16;
...
#[test]
fn trajectory_golden_diagonal_zig_zag_motion() {
    let mut state = RuntimeState::default();
    state.config.delay_event_to_smear = 0.0;
    state.config.simulation_hz = 240.0;
    state.config.max_simulation_steps_per_frame = 16;
...
#[test]
fn trajectory_golden_window_buffer_switch_motion() {
    let mut state = RuntimeState::default();
    state.config.delay_event_to_smear = 0.0;
    state.config.simulation_hz = 240.0;
    state.config.max_simulation_steps_per_frame = 16;
...
#[test]
fn trajectory_golden_scroll_while_animating() {
    let mut state = RuntimeState::default();
    state.config.delay_event_to_smear = 0.0;
    state.config.simulation_hz = 240.0;
    state.config.max_simulation_steps_per_frame = 16;
