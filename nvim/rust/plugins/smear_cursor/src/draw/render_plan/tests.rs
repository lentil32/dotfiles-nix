...
fn set_frame_corners(frame: &mut RenderFrame, corners: [Point; 4]) {
    ...
    frame.step_samples = vec![sample_for_corners(corners)];
}
...
#[test]
fn render_frame_to_plan_records_decode_path_trace_on_output_state() {
    let viewport = Viewport {
        max_row: 80,
        max_col: 160,
    };

    let output = render_frame_to_plan(&base_frame(), PlannerState::default(), viewport);
    let diagnostics = output.next_state.decode_diagnostics();

    assert_eq!(diagnostics.last_path, Some(output.decode_path));
    assert_eq!(
        diagnostics.path_counters.count_for(output.decode_path),
        1,
        "selected decode path should be counted on the planner state"
    );
    assert_eq!(
        diagnostics.path_counters.total(),
        1,
        "a single frame should contribute exactly one decode-path sample"
    );
}
...
#[test]
fn target_cell_is_punched_through() {
...
fn staged_deposit_scales_sheath_band_with_speed() {
    ...
    stationary_frame.step_samples = vec![sample_for_corners(stationary_frame.corners)];
