...
mod tests {
    use super::{
        ...
        SweepMaterializeScratch, comet_tail_profiles, compile_field, compile_field_from_cache,
        compile_field_from_cache_with_scratch, deposit_swept_occupancy, intensity_q16,
        materialize_swept_occupancy_with_scratch, max_comet_support_steps, prepare_swept_occupancy_geometry,
        prune_history, q16_from_non_negative, simulation_step_ms, tail_support_steps,
    };
    use crate::core::types::{ArcLenQ16, StepIndex, StrokeId};
    ...
    fn shared_sweep_geometry_matches_direct_deposit_for_tail_band_widths() {
        ...
        let geometry =
            prepare_swept_occupancy_geometry(start, end, 2.0, 1.25 * max_width_scale, 0.8 * max_width_scale);
        let mut scratch = SweepMaterializeScratch::default();
    ...
    fn shared_sweep_geometry_materialization_is_order_independent() {
        ...
        let max_width_scale = profiles
            ...
            .fold(0.0_f64, |max, profile| max.max(profile.width_scale));
        let geometry =
            prepare_swept_occupancy_geometry(start, end, 2.0, 1.0 * max_width_scale, 1.0 * max_width_scale);
