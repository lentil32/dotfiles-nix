use crate::lua::{
    ParsedOptionalChange, bool_from_object, f64_from_object, i64_from_object, invalid_key,
    parse_indexed_objects, parse_optional_change_with as parse_optional_change_value_with,
    parse_optional_with as parse_optional_value_with, string_from_object,
};
use crate::state::{
    ...
    for key in opts.keys() {
        ...
        if !known {
            return Err(invalid_key(
                key_str.as_ref(),
                "supported option key for rs_smear_cursor",
            ));
        }
    }
    Ok(())
